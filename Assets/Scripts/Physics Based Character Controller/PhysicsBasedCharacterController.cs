...
public class PhysicsBasedCharacterController : MonoBehaviour
{
  ...
  private void Awake()
  {
    ...
    _isometricAiming = GetComponent<IsometricAiming>();
  }

  private void OnEnable()
  {
    _inputListener.GetAction(InputActionType.Move).performed += MoveInputAction;
    _inputListener.GetAction(InputActionType.Move).canceled += MoveInputAction;
    _inputListener.GetAction(InputActionType.Jump).performed += JumpInputAction;
  }
  ...
  private Vector3 GetLookDirection(lookDirectionOptions lookDirectionOption)
  {
    ...
    else if (lookDirectionOption == lookDirectionOptions.isometricAim)
    {
      (bool success, Vector3 mousePos) = _isometricAiming.GetMousePosition();
